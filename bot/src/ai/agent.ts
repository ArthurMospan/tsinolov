...
export async function handleAgentQuery(userId: number, text: string): Promise<string> {
    try {
        ...
        const messages: any[] = [
            { 
                role: 'system', 
                content: 'Ти — AI-асистент "Сільпо Smart Watchlist". Допомагаєш гостям Сільпо керувати списком обраного, знаходити акції та вигідні товари (Smart Buy). Спілкуйся привітно, у стилі Сільпо (використовуй емодзі 🍊🛒🧡). Викликай необхідні інструменти (tools) для отримання даних. Аналізуй ціни та пропонуй найвигідніші варіанти.' 
            },
